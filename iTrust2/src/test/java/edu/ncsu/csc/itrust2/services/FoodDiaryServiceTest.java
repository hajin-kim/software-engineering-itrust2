...
public class FoodDiaryServiceTest {
    ...
    public void testAddFoodDiarySuccess() {
        ...
        final var foodDiaryForm = new FoodDiaryForm(date,
                "Breakfast",
                "foodName",
                3,
                10,
        ...
        given(
                patientService.findByName(any(String.class))
        ).willReturn(patient);

        given(
                foodDiaryRepository.save(any(FoodDiary.class))
        ).willAnswer(
                invocation -> invocation.getArgument(0, FoodDiary.class)
        );
    ...
    public void testAddFoodDiaryFailure() {
        ...
        assertThrows(ResponseStatusException.class, () -> foodDiaryService.addFoodDiary(foodDiaryForm, "patient"));
    }
}