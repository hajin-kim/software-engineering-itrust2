...
public class OphthalmologySurgery extends DomainObject {
    ...
    public boolean checkVisualAcuityResultValid(final Integer visualAcuityResult){
        ...
        if(leftVisualAcuityResult < 20 || leftVisualAcuityResult > 200){
            throw new IllegalArgumentException("visual acuity result must be the value between 20 and 200.");
        }
        return true;
    }
    ...
    public boolean checkSphereValid(final Float sphere){
        ...
        if(!Pattern.matches(pattern, String.valueOf(leftSphere))){
            throw new IllegalArgumentException("sphere must be the floating point number up to two digits.");
        }
        return true;
    }
