...
/**
 * Custom AuthenticationFailureHandler to record Failed attempts, and lockout or ban a user or IP if
 * necessary.
 *
 * @author Thomas
...
public class FailureHandler extends SimpleUrlAuthenticationFailureHandler {
        ...
        if (ae instanceof BadCredentialsException) {
            ...
            if (loginAttemptService.countByIP(addr) >= 5) {
                ...
                if (loginLockoutService.getRecentIPLockouts(addr) >= 2) {
                    ...
                    loggerUtil.log(
                            TransactionType.IP_BANNED, addr, null, addr + " has been banned.");
                    ...
                    loggerUtil.log(
                            TransactionType.IP_LOCKOUT,
                            addr,
                            null,
                            addr + " has been locked out for 1 hour.");
    ...
    private void sendEmail(final String username) {
        ...
        if (null != user) {
            loggerUtil.log(
                    TransactionType.ACC_LOCKOUT_EMAIL_NOTICE, loggerUtil.getCurrentUsername());
